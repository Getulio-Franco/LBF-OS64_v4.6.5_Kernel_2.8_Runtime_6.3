...
int sys_open(char *path) {
    if (!path || (uint64_t)path >= 0x8000000000000000ULL) {
        return -1;
    }

    // Remove o prefixo de disco (ex: "0:/INVADERS.CH8" -> "INVADERS.CH8")
    ...
    // Garante que o nó tem a tabela de operações do FAT32 associada
    if (!node->ops) {
    ...
    for (int i = 0; i < MAX_FILES_PER_PROCESS; i++) {
        if (!fd_table[i].used) {
            ...
            fd_table[i].offset = 0; // Inicia a leitura no byte 0
            fd_table[i].used = 1;
            
            if (node->ops && node->ops->open) {
                node->ops->open(node);
            }
            return i; // Retorna o FD (0, 1, 2...)
        }
    }
...
int sys_read(int fd, uint8_t *buf, uint32_t count) {
    // 1. Validação de Ring 3 e descritor
    if (fd < 0 || fd >= MAX_FILES_PER_PROCESS || !fd_table[fd].used) {
        return -1;
    }
    if (!buf || (uint64_t)buf >= 0x8000000000000000ULL) {
        return -1;
    }
    ...
    // 2. Chama o driver passando os parâmetros na ordem correta (node, offset, size, buffer)
    int bytes_lidos = node->ops->read(node, fd_table[fd].offset, count, buf);

    // 3. Se a leitura deu certo, avança o ponteiro de offset do arquivo
    if (bytes_lidos > 0) {
        fd_table[fd].offset += bytes_lidos;
    }
