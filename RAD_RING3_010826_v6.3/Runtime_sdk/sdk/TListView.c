...
TGUIControl* GUI_CreateListView(TGUIEnvironment* app, int x, int y, int w, int h, TNotifyEvent onChange) {
    ...
    // CORREÇÃO: O Scrollbar agora usa a mesma altura (h) e posição Y exata do ListView
    lv->VScrollBar = GUI_CreateScrollBar(app, x + w - sb_width, y, sb_width, h, 1);
    if (lv->VScrollBar) {
        lv->VScrollBar->Owner = lv;
        lv->VScrollBar->OnScroll = OnVScrollChange;
        GUI_ScrollBar_SetRange(lv->VScrollBar, 0, 0, h / 18);
    }

    // O ListView tem sua largura reduzida em 'sb_width' para abrir espaço para a barra
...
void GUI_ListView_AddItem(TGUIControl* lv, const char* name, uint32_t size, uint8_t attributes) {
    ...
    if (lv->VScrollBar) {
        ...
        // CORREÇÃO: Garante que o tamanho da página respeite o total de itens reais
        int page_size = (max_visible_lines > 0) ? max_visible_lines : 1;
        if (page_size > lv->LVItemCount) {
            page_size = lv->LVItemCount;
        }

        GUI_ScrollBar_SetRange(lv->VScrollBar, 0, max_scroll_val, page_size);
    }
}
...
void GUI_ListView_Clear(TGUIControl* lv) {
    ...
    if (lv->VScrollBar) {
        ...
        int max_visible_lines = (lv->Height - 4) / 18;
        int page_size = (max_visible_lines > 0) ? max_visible_lines : 1;
        GUI_ScrollBar_SetRange(lv->VScrollBar, 0, 0, page_size);
    }
}
