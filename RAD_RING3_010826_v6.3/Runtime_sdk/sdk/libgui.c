...
bool GUI_ProcessMouseClick(TGUIEnvironment* app, int mouse_x, int mouse_y) {
    ...
    for (int i = app->ControlCount - 1; i >= 0; i--) {
        ...
        if (mouse_x >= abs_x && mouse_x <= (abs_x + ctrl->Width) &&
            ...
            // =====================================================================
            // --- COMPONENTE SCROLLBAR (Tratamento com salto por PageSize) ---
            // =====================================================================
            if (ctrl->Type == TYPE_SCROLLBAR) {
                int metade_vertical = ctrl->Height / 2;
                int local_y = mouse_y - abs_y;
            
                // Define o tamanho do salto baseado na página visível (ex: 15 itens por vez)
            ...
            // =====================================================================
            // --- COMPONENTE LISTVIEW (Focado estritamente nos itens) ---
            // =====================================================================
            if (ctrl->Type == TYPE_LISTVIEW) {
                ...
                if (kernel_lv) {
                    // Como o Scrollbar agora é um controle separado acima na Z-Order,
                    // este bloco processa apenas cliques na área útil dos itens!
                    int local_y = mouse_y - abs_y - 4; 
                
                    // Pega a posição de rolagem atual diretamente do Kernel
                    int current_scroll = (int)gui_get_prop((void*)ctrl->KernelHandle, PROP_SCROLL_Y);

                    // O índice calculado considera o offset da rolagem
                    int calculated_index = (local_y / 18) + current_scroll;

                    // Trava os limites de segurança
                    ...
                    // Dispara o evento de clique no item
...
void GUI_ProcessKeyboard(TGUIEnvironment* app, char key) {
    ...
    // --- COMPONENTE TERMINAL/LOG (TMemo) ---
        ...
        // Encapsula chamando diretamente a rota segura com realloc automático
